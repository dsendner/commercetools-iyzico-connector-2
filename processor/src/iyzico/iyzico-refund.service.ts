...
import { IyzicoRefundRequest, IyzicoRefundResponse, iyzicoRefundResponseSchema } from './contracts/refund.schema';
...
export class IyzicoRefundService {
    ...
    async refund(paymentId: string, action: RefundPaymentAction): Promise<PaymentIntentResponse> {
        const startedAt = Date.now();
        this.logger.log(JSON.stringify({ event: 'iyzico.refund.requested', paymentId }));

        const payment = await this.ctPayment.getPayment({ id: paymentId });
        const refundReference = action.merchantReference ?? `refund-${payment.id}-${action.amount.centAmount}`;

        if (this.hasCompletedRefund(payment, refundReference, action)) {
            this.logger.log(JSON.stringify({ event: 'iyzico.refund.already_completed', paymentId }));
        ...
        const pendingRefunds = itemTransactions
            ...
            .map((item) => ({
                item,
                request: this.buildRefundRequest(action, item, refundReference),
            }));
        this.logger.log(
            JSON.stringify({
        ...
        const results = await Promise.allSettled(
            pendingRefunds.map(async ({ request }) => {
                const rawResponse = await this.iyzico.post<unknown>(REFUND_ENDPOINT, request);
        ...
        const pendingRefundIterator = pendingRefunds.values();
        const pspInteractions = results.map((result, index) => {
            const { item, request } = pendingRefundIterator.next().value as (typeof pendingRefunds)[number];
            const response =
            ...
            return connectPaymentsSdk.GenerateInterfaceInteractionCustomFieldsDraft({
                createdAt: new Date().toISOString(),
                interactionId: response.paymentTransactionId ?? item.paymentTransactionId,
        ...
        const allSuccessful = results.every((result) => result.status === 'fulfilled' && result.value.status === 'success');
        ...
        const successfulItemCount = results.filter(
        ...
        const completionLog = JSON.stringify({
            ...
            outcome: allSuccessful ? PaymentModificationStatus.Approved : PaymentModificationStatus.Rejected,
            paymentId,
            successfulItemCount,
        });
        ...
        const technicalFailure = results.find((result) => result.status === 'rejected');
        if (technicalFailure?.status === 'rejected') {
            if (technicalFailure.reason instanceof Error) throw technicalFailure.reason;
    ...
    private buildRefundRequest(
        ...
        return {
            conversationId: `${refundReference}-${item.itemId}`,
    ...
    private getItemTransactions(
        ...
        for (const interaction of [...(payment.interfaceInteractions ?? [])].reverse()) {
            const type = String(interaction.fields?.type ?? '');
            if (type !== 'iyzico-confirm-success' && type !== 'iyzico-refill-success') continue;
            if (charge.interactionId && interaction.fields?.interactionId !== charge.interactionId) continue;
        ...
        throw new BadRequestException(`Payment ${payment.id} has no Iyzico item transaction references`);
    }
    ...
    private getSuccessfulCharge(
        ...
        if (action.transactionId) {
            const charge = successfulCharges.find((transaction) => transaction.id === action.transactionId);
            if (!charge) {
    ...
    private hasCompletedRefund(
        ...
        return payment.transactions.some(
            (transaction) =>
                ...
                transaction.interactionId === refundReference &&
    ...
    private parseRefundResponse(response: unknown): IyzicoRefundResponse {
        ...
        if (!parsed.success) {
            ...
            throw new BadGatewayException('Iyzico returned an invalid refund response');
        }
        return parsed.data;
    }
    ...
    private validatePayment(payment: connectPaymentsSdk.Payment, action: RefundPaymentAction): void {
        if (payment.paymentMethodInfo.paymentInterface !== 'iyzico') {
            throw new BadRequestException(`Payment ${payment.id} is not an Iyzico payment`);
        }
        ...
        if (
            ...
            throw new BadRequestException('Iyzico refunds currently support the full payment amount only');
        }
        ...
        if (otherCompletedRefund) {
            throw new BadRequestException(`Payment ${payment.id} is already refunded`);
        }
    }

    private validateSuccessfulResponse(response: IyzicoRefundResponse, request: IyzicoRefundRequest): void {
        ...
        if (
            ...
            throw new BadGatewayException('Iyzico refund response does not match the request');
        }

        if (
            !this.iyzico.verifyResponseSignature(
                [response.paymentId, response.price, response.currency, response.conversationId],
                response.signature,
            )
        ) {
            throw new BadGatewayException('Iyzico refund response has an invalid signature');
        }
    }
}
